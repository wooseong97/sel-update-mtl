...
def save_model_pred_for_one_task(p, sample, output, save_dirs, task=None, epoch=None):
    ...
    for jj in range(int(inputs.size()[0])):
        ...
        if len(output_task.shape)==2: pred = output_task
        else: pred = output_task[jj] # (H, W) or (H, W, C)
        ...
        if not pred.shape[:2] == (im_height, im_width):
            pred = torch.unsqueeze(torch.unsqueeze(pred, dim=0),dim=0)
            pred = F.interpolate(pred, size=(im_height.item(), im_width.item()), mode='bilinear')

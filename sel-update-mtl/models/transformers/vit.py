...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, select_list, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx + 1 in self.select_list: 
                selected_fea.append(x[:, 1:])

        x = self.norm(x)[:, 1:]
        selected_fea.append(x)

        return x, selected_fea
...
def resize_pos_embed2(posemb, posemb_new, num_tokens=1, gs_new=(), img_size=None, patch_size=None) :
    # Rescale the grid of position embeddings when loading from state_dict. Adapted from
    # https://github.com/google-research/vision_transformer/blob/00883dd691c63a6830751563748663526e811cee/vit_jax/checkpoint.py#L224
    ...
    gs_old = int(math.sqrt(len(posemb_grid)))
    if not len(gs_new):  # backwards compatibility
        gs_new = list(map(lambda k: int(k/patch_size), img_size))
        # gs_new = [int(math.sqrt(ntok_new))] * 2
    assert len(gs_new) >= 2
    _logger.info('Position embedding grid-size from %s to %s', [gs_old, gs_old], gs_new)
    ...
    if posemb_grid.dtype is torch.float32:
        posemb_grid = F.interpolate(posemb_grid, size=gs_new, mode='bicubic', align_corners=False)
    else:
        posemb_grid = F.interpolate(torch.tensor(posemb_grid, dtype=torch.float32), size=gs_new, mode='bicubic', align_corners=False)
...
def vit_large_patch16_384(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(select_list = [6, 12, 18], patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_384', pretrained=pretrained, **model_kwargs)
    return model


def vit_large_patch14_384(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(select_list = [6, 12, 18], patch_size=14, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_384', pretrained=pretrained, **model_kwargs)
    return model
...
def vit_base_patch16_384(pretrained=False, **kwargs):
    ...
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(select_list = [3, 6, 9], patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
...
def vit_small_patch16_384(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(select_list = [3, 6, 9], patch_size=16, embed_dim=384, depth=12, num_heads=12, **kwargs)
...
def vit_tiny_patch16_384(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(select_list = [3, 6, 9], patch_size=16, embed_dim=192, depth=12, num_heads=3, **kwargs)

...
class Resize:
    ...
    def scale(self, key, unscaled, size):
        """Randomly scales image and label.
        Args:
            key: Key indicating the uscaled input origin
            unscaled: Image or target to be scaled.
    ...
    def __call__(self, sample):
        for key, val in sample.items():
            if key == 'meta':
                continue
            sample[key] = self.scale(key, val, size=self.size)
        return sample
...
class RandomCrop:
    ...
    def __call__(self, sample):
        ...
        if self.cat_max_ratio < 1.:
            # Repeat 10 times
            for _ in range(10):
                sample_uncropped = sample[list(sample.keys())[1]]
                seg_tmp = self.random_crop('semseg', sample_uncropped, crop_location)

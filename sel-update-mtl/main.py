...
from utils.train_utils import train_phase, train_phase_affinity, form_task_affinity
...
def main():
    ...
    # Get model
    model = get_model(p)
    print("#"*10); print("Numer of Param"); print(count_parameters(model)); print("#"*10)
    ...
    # Optimizer
    scheduler, optimizer = get_optimizer(p, model, criterion=criterion)

    # Performance meter init
    if p['train_db_name']=='Taskonomy': pass
    else: performance_meter = PerformanceMeter(p, p.TASKS.NAMES)
    ...
    if os.path.exists(p['checkpoint']) or args.run_mode == 'infer':
        if args.trained_model != None: checkpoint_path = args.trained_model
        else: checkpoint_path = p['checkpoint']
        if args.local_rank == 0:
        ...
        if 'scheduler' in checkpoint.keys(): scheduler.load_state_dict(checkpoint['scheduler'])
        if 'epoch' in checkpoint.keys(): start_epoch = checkpoint['epoch'] + 1
        else: start_epoch = 0
        if 'iter_count' in checkpoint.keys(): iter_count  = checkpoint['iter_count']
        else: iter_count = 0
    else:
        if args.local_rank == 0: 
            print('Fresh start...')
    ...
    if 'loss_schema' in p.keys() and p['loss_schema'] in ['affinity']:
        task_affinity = form_task_affinity(p)
    ...
    if args.run_mode != 'infer':
        for epoch in range(start_epoch, p['epochs']):
            ...
            if args.local_rank == 0:
                ...
                print('-'*10)
            if 'loss_schema' in p.keys() and p['loss_schema']=='affinity':
                print("affinity")
                end_signal, iter_count = train_phase_affinity(p, args, train_dataloader, test_dataloader, model, criterion, optimizer, scheduler, epoch, tb_writer_train, tb_writer_test, iter_count, task_affinity)
            else:

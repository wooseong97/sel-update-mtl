...
class TaskonomyDataset(data.Dataset):
    def __init__(self, img_types, data_dir='', 
        ...
        if split == 'medium':
            ...
            self.data = loadSplit(splitFile = os.path.join(data_dir, 'splits_taskonomy/train_val_test_custom.csv'))
        else:
            assert False
...
if __name__ == '__main__':
    ...
    train_set = TaskonomyDataset(img_types, split='custom', partition='train', resize_scale=256, crop_size=224, fliplr=True)
    print(len(train_set))
